...
class PPO_buffer:
	...
	def finish_path(self, last_val):
		...
		rewards = np.append(self.rew_buf[path_slice],
							last_val)  # final reward estimate or 0 for completion of trajectory
		...
		normalized_discounted_advantage_values = discounted_advantage_values - discounted_advantage_values_mean / (
					1e-8 + discounted_advantage_values_std)
...
class PPO_clip_agent():
	def __init__(self, observation_space, action_space, PolicyNetworkDims, VNetworkDims, buffer_size,
				 discount_factor=0.99, lambda_=0.97, v_lr=0.001, p_lr=0.0001, train_pi_iters=80, train_v_iters=80,
	...
	def run(self,
		...
		for epoch in range(epochs):
			...
			for t in range(steps_per_epoch):
				...
				action, value_estimate, logp = self.take_action(torch.as_tensor(o, dtype=torch.float32))
				...
				self.buffer.store(o, action, reward, value_estimate, logp)
				o = next_o
				...
				if terminal or epoch_ended:
					if timeout or epoch_ended:  # Getting a final estimate of V since trajectory didn't finish
						_, v, _ = self.take_action(torch.as_tensor(o, dtype=torch.float32))
					else:
						v = 0
